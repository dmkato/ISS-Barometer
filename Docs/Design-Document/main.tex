...
For the ISS Barometer the main concerns are in regards to the maintainability of the code and the way each element interacts.
Each of the main components of the application will need to interact in a manner that is easy follow and manipulate.
...
Separating out each of the pages as views will allow us to keep the code separate and more organized, as well as letting us reuse the graph view to display it in full screen.
To address the concern of transfering data between views, we will use segues as outlined in the Coding Explorer Blog \cite{dataThroughSeque}.
...
To store each barometer reading, a member variable of the main view controller will be created that holds an array of all of the collected datapoints in the current session.
...
The stakeholders of this would be our client, end users and astronauts aboard the ISS.
The astronauts will need to rely on our app to correctly have all the timing work in unison.
...
The barometer might be reading the data slower than the graph can display it, also it might be reading it faster than we can display the data on the graph.
This can cause problems with the end users.
Looking at the graph with a older atmospheric pressure than the one the barometer can cause the user to not evacuate early, this could result in death of the crew.
...
Astronauts need to have all data be precise and completely accurate to save lives.
...
This means that we can display the atmospheric pressure, dp/dt, calculate time to evacuation and display the graph simultaneously making it accurate and safe.
...
This is the best solution as the testing suit is set up when the project is created, making it easy to test our program from the beginning.
